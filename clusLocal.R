...
#
##################################################################################################
gatherFilesFoldsLocal <- function(ds, dataset_name, Folder10F, FolderLocal){
  ...
  s = 1
  gfflParalel <- foreach(s = 1:10) %dopar% {
    cat("\nFold: ", s)
...
#
##################################################################################################
splitLabels <- function(namesLabels, ds, dataset_name, Folder10F, FolderLocal){
  ...
  f = 1
  slParalel <- foreach (f = 1:10) %dopar%{
  #while(f<=10){
    cat("\nFold: ", f)
...
#
##################################################################################################
gatherLocalPredicts <- function(namesLabels, ds, dataset_name, FolderLocal){
  ...
  f = 1
  glpParalel <- foreach (f = 1:10) %dopar%{
  #while(f<=10){
    library("foreign")
    cat("\nFold: ", f)             
    FolderSplit = paste(FolderLocal, "/Split-", f, sep="")    
  ...
  gc()
  cat("\n##################################################################################################")
  cat("\n# FIM DA FUNCAO GATGHER LOCAL PREDICTS                                                           #") 
  cat("\n##################################################################################################")
  cat("\n\n\n\n")
}
...
#
##################################################################################################
avaliaLocal <- function(ds, dataset_name, Folder){  
  ...
  gc()
  cat("\n##################################################################################################")
  cat("\n# FIM DA FUNCAO AVALIA LOCAL CLUS                                                                #") 
  cat("\n##################################################################################################")
  cat("\n\n\n\n")
}
...
#
##################################################################################################
gatherEvalLocal <- function(ds, dataset_name, Folder){
  ...
  gc()  
  cat("\n##################################################################################################")
  cat("\n# FIM DA FUNCAO GATHER EVAL LOCAL                                                                #") 
  cat("\n##################################################################################################")
  cat("\n\n\n\n")
}
...
#
##################################################################################################
clusLocal <- function(ds, dataset_name, Folder10F, FolderLocal, namesLabels){ 
  
  cat("\nLOCAL: Tests each Splits Localy\n")
  
  cat("\nLOCAL: Joins training and test files in a single folder for running the clus\n")
  timeGatherFiles = system.time(gatherFilesFoldsLocal(ds, dataset_name, Folder10F, FolderLocal))
  
  cat("\nLOCAL: Split the labels\n")
  timeSplitLabels = system.time(splitLabels(namesLabels, ds, dataset_name, Folder10F, FolderLocal))
  ...
  timeGatherPreds = system.time(gatherLocalPredicts (namesLabels, ds, dataset_name, FolderLocal))
  
  cat("\nLOCAL: Evaluates the split classification\n")
  timeEvalLocal = system.time(avaliaLocal(ds, dataset_name, FolderLocal))
  
  cat("\nLOCAL: Gather Evaluated Measures\n")
  timeGE = system.time(gatherEvalLocal(ds, dataset_name, FolderLocal))
  
  cat("\nLOCAL: Save Runtime\n")
  RunTimeLocal = rbind(timeGatherFiles, timeSplitLabels, timeClusLocal, timeGatherPreds, timeEvalLocal, timeGE)
  setwd(FolderLocal)
  write.csv(RunTimeLocal, "RunTimeLocal.csv")
  
  gc()
  cat("\n##################################################################################################")
  cat("\n# FIM DA FUNCAO CLUS LOCAL                                                                       #") 
  cat("\n##################################################################################################")
  cat("\n\n\n\n")
}