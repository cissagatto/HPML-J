...
generatedRandomPartitions <- function(ds, namesLabels, dataset_name, FolderRandom, number_folds){
  ...
  #while(f<=number_folds){
    ...
    while(k<=num.particoes){
      ...
      final = data.frame(cbind(particao3, rotulos2))
      setwd(FolderPartition)
      write.csv(final, "particao.csv")
      
      # ASSOCIA OS RÓTULOS COM OS ÍNDICES E OS GRUPOS
      #final2 = data.frame(cbind(particao3$grupo, rotulos2$rotulos))
      final2 = data.frame(cbind(particao3[, 1, drop = FALSE], rotulos2[,1, drop = FALSE]))
      names(final) = c("grupo","rotulo")
      setwd(FolderPartition)
      write.csv(final2, "particaoFinal.csv")
      
      grupos.labels = vector("list", k)
      for(i in 1:k){
        grupos.labels[[i]] = final$rotulos[final$grupo == i]
      }
...
clusRandom <- function(ds, namesLabels, dataset_name, FolderDSF, FolderResDataset,
  ...
  timeSplit = system.time(generatedRandomPartitions(ds, namesLabels, 
                                dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: build and validate partitions")  
  timeBuild = system.time(RandomPartitions(ds, namesLabels, dataset_name, 
                                           FolderRandom, FolderDSF, number_folds))
  
  cat("\nRANDOM: gather partitions")  
  timePreds = system.time(gatherPredsRandom(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: eval partitions")  
  timeEval = system.time(evalRandom(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: gather eval partitions")  
  timeGather = system.time(gatherEvaluations(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: gather f1 partitions")  
  timeF1 = system.time(gatherF1macro(ds, dataset_name, FolderRandom, number_folds))
  ...
  cat("\nRANDOM: test clus")  
  timeMount = system.time(mountRandomPart(ds, dataset_name, FolderDSF, 
                                          FolderRandom, number_folds))
  
  cat("\nRANDOM: predictions")  
  timePredSplit = system.time(splitsPredictionsRandom(ds, dataset_name, FolderDSF, 
                                                  FolderRandom, number_folds))
  
  cat("\nRANDOM: gather predictions")  
  timePredGather = system.time(gatherPredsRandom2(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: evaluation")  
  timePredGather = system.time(evalRandom2(ds, dataset_name, FolderRandom, number_folds))
  ...
  RunTimeRandom = rbind(timeSplit, timeBuild, timePreds, timeEval,
                        timeGather, timeF1, timeMount, timePredSplit, timePredGather, 
                        timePredGather)
