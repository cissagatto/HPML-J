...
generatedRandomPartitions <- function(ds, namesLabels, dataset_name, FolderRandom, number_folds){
...
# 
##################################################################################################
RandomPartitions <- function(ds, namesLabels, dataset_name, 
...
# 
##################################################################################################
gatherPredsRandom <- function(ds, dataset_name, FolderRandom, number_folds){
...
# 
##################################################################################################
evalRandom <- function(ds, dataset_name, FolderRandom, number_folds){
...
# 
##################################################################################################
gatherEvaluations <- function(ds, dataset_name, FolderRandom, number_folds){
...
# 
##################################################################################################
gatherF1macro <- function(ds, dataset_name, FolderRandom, number_folds){
...
###############################################################################################
###############################################################################################
mountRandomPart <- function(ds, dataset_name, FolderDSF, FolderRandom, number_folds){
...
splitsPredictionsRandom <- function(ds, dataset_name, FolderDSF, FolderRandom, number_folds){
...
gatherPartitions <- function(ds, namesLabels, number_folds, FolderRandom){
...
clusRandom <- function(ds, namesLabels, dataset_name, FolderDSF, FolderResDataset,
  ...
  timeSplitVal = system.time(generatedRandomPartitions(ds, namesLabels, 
                                dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: summary partitions")  
  timeGPTest = system.time(gatherPartitions(ds, namesLabels, number_folds, FolderRandom))
  
  cat("\nRANDOM: build and validate partitions")  
  timeBuildVal = system.time(RandomPartitions(ds, namesLabels, dataset_name, 
                                           FolderRandom, FolderDSF, number_folds))
  
  cat("\nRANDOM: gather predictions partitions")  
  timePredsVal = system.time(gatherPredsRandom(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: eval partitions")  
  timeEvalVal = system.time(evalRandom(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: gather eval partitions")  
  timeGatherVal = system.time(gatherEvaluations(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: gather f1 partitions")  
  timeF1Val = system.time(gatherF1macro(ds, dataset_name, FolderRandom, number_folds))
  ...
  cat("\nRANDOM: test clus")  
  timeMountTest = system.time(mountRandomPart(ds, dataset_name, FolderDSF, 
                                          FolderRandom, number_folds))
  
  cat("\nRANDOM: predictions")  
  timePredsTest = system.time(splitsPredictionsRandom(ds, dataset_name, FolderDSF, 
                                                  FolderRandom, number_folds))
  
  cat("\nRANDOM: gather predictions")  
  timeGatherTest = system.time(gatherPredsRandom2(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: evaluation")  
  timeEvalTest = system.time(evalRandom2(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: delete files unecessary")  
  timeDelete = system.time(deleteRAndomFiles(ds, dataset_name, FolderRandom, number_folds))
  
  cat("\nRANDOM: Save Runtime")
  RunTimeRandom = rbind(timeSplitVal, timeBuildVal, timePredsVal, timeEvalVal, timeGatherVal, timeF1Val,
                        timeMountTest, timePredsTest, timeGatherTest, timeEvalTest, timeGPTest, timeDelete)
