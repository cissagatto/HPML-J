...
#     None                                                                                       #
##################################################################################################
hybridPartitions <- function(ds, dataset_name, Folder10F, FolderHClust, FolderHybPart){  
  ...
  Folder10Tr = paste(Folder10F, "/Tr", sep="")
  dirFolder10Tr = c(dir(Folder10Tr))
  n_dirFolder10Tr = length(dirFolder10Tr)
  
  Folder10Vl = paste(Folder10F, "/Vl", sep="")
  dirFolder10Vl = c(dir(Folder10Vl))
  n_dirFolder10Vl = length(dirFolder10Vl)  
  
  dataset_name = dataset_name
  fold = c(0)
  numeroDeParticoes = c(0)
  numeroDeGrupos = c(0)
  grupo = c(0)
  rotulosDoGrupo = c(0)
  SumarioParticoes = data.frame(dataset_name, fold, numeroDeParticoes, numeroDeGrupos, grupo, rotulosDoGrupo)
  
  gr = c(0)
  TodasParticoes = data.frame(gr)
  
  f = 1  
  hybridParalel <- foreach(f = 1:10) %dopar%{  
    ...
    while(p<=num.part){
      ...
      if(totalGroupsPart == 1){
        ...
        while(g<=totalGroupsPart){
          ...
          setwd(Folder10Tr)
          nomeTr2 = paste(Folder10Tr, "/", nomeTr, sep="")
          ...
          setwd(Folder10Vl)
          nomeVl2 = paste(Folder10Vl, "/", nomeVl, sep="")
          ...
          ####################################################################################
          cat("\nSave information partitions\n")
          fold = f
          numeroDeParticoes = p
          numeroDeGrupos = as.numeric(totalGroupsPart)
          grupo = g
          rotulosDoGrupo = as.numeric(totalLabelsThisGr)
          SumarioParticoes = rbind(SumarioParticoes, data.frame(dataset_name, fold, numeroDeParticoes, 
                                                                numeroDeGrupos, grupo, rotulosDoGrupo))
          
          setwd(FolderSplitPartGroup)
          namae = paste("SummaryPartition-", g, ".csv", sep="")
          write.csv(SumarioParticoes, namae, row.names = FALSE)
  ...
  gc()
  cat("\n##################################################################################################")
  cat("\n# FIM DA FUNCAO MOUNT AND VALIDATE HYBRID PARTITION                                              #")
  cat("\n##################################################################################################")
  cat("\n\n\n\n")
}
